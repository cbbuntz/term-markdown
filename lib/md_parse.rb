...
module MD
  ...
  # May need to rely on \e[{n}G for correct alignment
  def tableize(buffer)
    table = %r[
      ^([^\|\n]*\|)+[^\n]*\n
      ^[ \t]*-{3,}:?(\s*\|\s*:?-{3,}:?)+\n
  ...
  def pre_process(s)
    ...
    a = s.split(/\n/).map do |line|
      if (line =~ /^ {4,}/)
        ...
        line.replace(process_superscript(line))
      end
      prefix(line)
      line
    end
...
s = "111111 This is a test string. 1234567890. 222222 This is a test string. 1234567890. 333333 This is a test string. 1234567890. 444444 This is a test string. 1234567890. 555555 This is a test string. 1234567890."


def word_wrap_ary(s, maxlen)
  post = s.slice!(maxlen..-1)
  # pre, space, mid = s.rpartition(/\b(?!$)/)
  pre, space, mid = s.rpartition(/ +/)
  pre.rstrip!
  post = post.prepend(mid).lstrip
  if post.length > maxlen
   post = word_wrap_ary(post, maxlen)
  end
  [pre, post].flatten
end

def word_wrap(s, lmargin, rmargin, tabstop = 4)
  maxlen = rmargin - lmargin
  margin = ' ' * lmargin
  s.gsub!('\t', ' ' * tabstop)

  word_wrap_ary(s, maxlen)
    .map do |line|
    margin + line
  end
    .join("\n")
